...
class EmpAbsent extends JFrame
{
	EmpAbsent(String uname)
	{
		...
		JLabel l1 = new JLabel("EMPLOYEE ABSENT DATES");
		l1.setBounds(250,20,1000,80);
		...
		try
		{
			...
			ResultSet rs = st.executeQuery("select * from empabsentdate where uid='"+uname+"';");
			DefaultTableModel model = new DefaultTableModel();
			JTable table = new JTable(model);
			model.addColumn("USername(ID)");
			model.addColumn("Absent Date");
